...
st.set_page_config(page_title="IRN-PROMEDIO", page_icon=":bar_chart:", layout="wide")

st.title("Promedio por mes, del Indicador de Rendimiento Neto (IRN) de las Siefores")
st.subheader("")
st.subheader("")
...
st.title(":bar_chart: IRN-PROMEDIO")
st.markdown("##")
...
t4 = round((df_selection["IRN-PROMEDIO"].sum()),3)
...
f3 = df_selection.groupby(by=["AFORE"])[["IRN-PROMEDIO"]].sum().sort_values(by="IRN-PROMEDIO")
fig3 = px.bar(
    f3,
    x="IRN-PROMEDIO",
    y=f3.index,
    orientation="h",
    title="<b>AFORE e IRN-PROMEDIO </b>",
    color_discrete_sequence=["#621132"] * len(f3),
    template="plotly_white",
)
fig3.update_layout(
