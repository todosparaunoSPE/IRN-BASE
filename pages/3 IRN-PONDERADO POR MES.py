...
st.title("Indicador de Rendimiento Neto (IRN) ponderado de las Siefores")
st.subheader("")
st.subheader("")
...
st.title(":bar_chart: IRN-PONDERADO")
st.markdown("##")
...
t4 = round((df_selection["IRN-PONDERADO"].sum()),3)
...
f3 = df_selection.groupby(by=["AFORE"])[["IRN-PONDERADO"]].sum().sort_values(by="IRN-PONDERADO")
fig3 = px.bar(
    f3,
    x="IRN-PONDERADO",
    y=f3.index,
    orientation="h",
    title="<b>AFORE e IRN-PONDERADO </b>",
    color_discrete_sequence=["#621132"] * len(f3),
    template="plotly_white",
)
fig3.update_layout(
